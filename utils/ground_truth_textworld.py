...
def process_local_facts(info_game, info_facts):
    ...
    state = State(kb.logic, info_facts)
...
def process_fully_obs_facts(facts, info_game):
    kb = textworld.Game.deserialize(info_game).kb

    state = State(kb.logic, facts)
...
def serialize_facts(facts):
    ...
    CONSTANT_NAMES = {"P": "player", "I": "inventory", "ingredient": None, "slot": None, "RECIPE": "recipe"}
    ...
    return [fact for fact in serialized if None not in fact]
