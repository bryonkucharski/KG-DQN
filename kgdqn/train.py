...
if __name__ == "__main__":
    ...
    game_name = 'new_game_test'
    folder_path = '../twgames/home_theme/new/' + game_name + "/"
    game_file = folder_path + "/" + game_name + ".ulx"
    ...
    param_grid.update(drqa_params)
   
    #grid_search = RandomGridSearch(param_grid, 0.2, 21)
    #all_params = grid_search.get_configs()
    #print(all_params)
    
    all_params = utils.grid_search.generate_cartesian_product(param_grid) 
    #parallelize(game_file, all_params[0])

    #Uncomment and define cuda visible device to parallelize across multiple processes

    Parallel(n_jobs=5, prefer='processes', backend='multiprocessing')(
        delayed(parallelize)(game_file, params) for params in all_params)
    '''
    params = {
        'replay_buffer_type': 'priority',
        'replay_buffer_size': 100000,
